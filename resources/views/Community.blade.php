...
<div class="avatar-circle me-2 d-flex align-items-center justify-content-center rounded-circle bg-secondary text-white" style="width: 80px; height: 80px;">
    @if ($post->imgProfile)
        <img class="img-fluid rounded-circle w-100 h-100 object-fit-cover" src="{{ Storage::url($post->imgProfile) }}" alt="">
    @else
...
<form action="{{ route('Community.report', ['id' => $post->id]) }}" method="POST">
    ...
    <select name="report_comment" required>
        <option value="" disabled selected>Seleziona il motivo della segnalazione</option>
...
<button
  ...
  @endif
  type="submit"
  class="btn btn-Report my-3">Segnala
</button>
</form>
                    @endforeach
